"""
Extração retroativa 23/04–31/05/2026: ~156k leads com concorrência e checkpoint.
...
- Checkpoint por página em checkpoint_retroativo.json (retoma se interrompido)
- Batch upsert de 100 leads por vez no Supabase
- Proteção de checkpoint: só avança se len(batch) > 0
"""
...
CHECKPOINT_FILE = "checkpoint_retroativo.json"
START_DATE = "2026-04-23"
END_DATE   = "2026-05-31"
MAX_WORKERS = 10
TOTAL_ESTIMADO = 156_000
...
def _salvar_checkpoint(cursor: str | None, batch_size: int) -> None:
    if batch_size == 0:
        logger.warning("Batch vazio — checkpoint NÃO avançado (proteção anti-falha silenciosa)")
        return
...
def main() -> None:
    ...
    if cursor:
        ...
        logger.info("Iniciando extração retroativa %s → %s com %d workers", START_DATE, END_DATE, MAX_WORKERS)
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        for perfis, proximo_cursor in klaviyo.paginar_contatos(START_DATE, END_DATE, cursor):
            ...
            _salvar_checkpoint(proximo_cursor, len(batch))
            total += len(batch)
